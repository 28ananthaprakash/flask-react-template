...
class BaseTestComment(unittest.TestCase):
    ...
    def tearDown(self) -> None:
        # Clean up test data
        try:
            # Try to clean comment collection if it exists
    ...
    def get_access_token(self, username: str = None, password: str = None) -> str:
        with app.test_client() as client:
            response = client.post(
                ...
                data=json.dumps({"username": username or self.DEFAULT_USERNAME, "password": password or self.DEFAULT_PASSWORD}),
            )
    ...
    def make_authenticated_request(self, method: str, account_id: str, task_id: str, token: str, comment_id: str = None, data: dict = None):
        if comment_id:
    ...
    def assert_comment_response(self, response_json: dict, expected_comment=None, **expected_fields):
    ...
    def assert_error_response(self, response, expected_status: int, expected_error_code: str = None):
